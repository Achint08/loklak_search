...
/**
 * @function feedsWithOnlyImages
 * Takes feeds as an argument and return feeds
 * with only images
 *
 * @arg: feeds: Feeds to be filtered
 */

export function feedsWithOnlyImages(feeds: ApiResponseResult[]): ApiResponseResult[] {
	const filteredFeeds: ApiResponseResult[] = [];
	feeds.forEach((feed) => {
		if (feed.images_count) {
			filteredFeeds.push(feed);
		}
	});
	return filteredFeeds || feeds;
